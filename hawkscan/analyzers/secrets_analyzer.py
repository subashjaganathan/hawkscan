...
_PATTERNS: list[tuple[re.Pattern, str, Severity, str]] = [
    ...
    # --- SaaS / developer tokens (distinctive formats -> low FP) ---
    (re.compile(r"\b(?:sk|rk)_live_[0-9A-Za-z]{24,}\b"),
     "Stripe live secret key", Severity.HIGH, "secret"),
    (re.compile(r"\bglpat-[0-9A-Za-z_\-]{20,}\b"),
     "GitLab personal access token", Severity.HIGH, "secret"),
    (re.compile(r"\bshp(?:at|ss|ca|pa)_[0-9a-fA-F]{32}\b"),
     "Shopify access token", Severity.HIGH, "secret"),
    (re.compile(r"\bnpm_[0-9A-Za-z]{36}\b"),
     "npm access token", Severity.HIGH, "secret"),
    (re.compile(r"\bSG\.[A-Za-z0-9_\-]{22}\.[A-Za-z0-9_\-]{43}\b"),
     "SendGrid API key", Severity.HIGH, "secret"),
    (re.compile(r"\bSK[0-9a-fA-F]{32}\b"),
     "Twilio API key", Severity.MEDIUM, "secret"),
    (re.compile(r"\bsk-ant-[A-Za-z0-9_\-]{20,}\b"),
     "Anthropic API key", Severity.HIGH, "secret"),
    (re.compile(r"\bsk-[A-Za-z0-9]{20}T3BlbkFJ[A-Za-z0-9]{20}\b"),
     "OpenAI API key", Severity.HIGH, "secret"),
    (re.compile(r"\bpypi-" + r"AgEIcHlwaS5vcmc" + r"[A-Za-z0-9_\-]{50,}"),
     "PyPI upload token", Severity.HIGH, "secret"),
    (re.compile(r"https://(?:discord(?:app)?\.com)/api/webhooks/\d+/[\w\-]+", re.I),
     "Discord webhook URL", Severity.MEDIUM, "secret"),
    (re.compile(r"\b\d{8,10}:[A-Za-z0-9_\-]{35}\b"),
     "Telegram bot token", Severity.MEDIUM, "secret"),
    # --- Credentials embedded in connection strings ---
    (re.compile(r"\b(?:mongodb(?:\+srv)?|postgres(?:ql)?|mysql|redis|amqp)://"
                r"[^\s:@/]+:[^\s:@/]+@[^\s/]+", re.I),
     "Database connection string with credentials", Severity.MEDIUM, "secret"),
    (re.compile(r"Authorization:\s*Bearer\s+[A-Za-z0-9_\-\.=]{20,}", re.I),
     "Hardcoded bearer token", Severity.LOW, "secret"),
